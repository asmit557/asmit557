# 👨‍🗋️ Asmit Verma – Tech Enthusiast, Developer & DSA Lover 🚀

![Banner](https://raw.githubusercontent.com/asmit557/asmit557/main/assets/banner.gif)

---

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&pause=1000&color=F97316&center=true&width=435&lines=Hey%2C+I'm+Asmit+Verma!;B.Tech+@+MNNIT;Full-Stack+Developer+%7C+Tech+Fanatic+%7C+DSA+Enthusiast"/>
</p>

---

## 🧠 About Me

* 🎓 Final year B.Tech student at **Motilal Nehru National Institute of Technology (MNNIT) Allahabad**.
* 💻 Passionate **Full-Stack Developer** with hands-on experience in building real-world projects.
* 🧩 Solved **550+ problems** on **LeetCode** with a **1600+ rating**, earned **6 LeetCode badges**.
* 🌱 Currently learning advanced backend architecture and WebSockets.
* 💬 Ask me about **React**, **Node.js**, **MongoDB**, **Tailwind**, **DSA**, or even a bit of **ZEGOCLOUD** magic.

---

## 🚀 Tech Stack

### 👨‍🔧 Languages & Fundamentals

`JavaScript` `TypeScript` `C++` `HTML` `CSS`

### 🌐 Frontend

`React.js` `Redux Toolkit` `Tailwind CSS` `Bootstrap` `Chart.js` `React Router DOM` `React Icons`

### ⚙️ Backend & DevOps

`Node.js` `Express.js` `MongoDB` `Mongoose` `Socket.IO` `JWT` `Nodemailer` `Cloudinary` `Razorpay`

### 🧪 Tools & Utilities

`Axios` `React Toastify` `Postman` `ZEGOCLOUD` `Git` `GitHub` `Nodemon` `Multer` `Helmet`

### 🛠️ Others

`SendGrid` `OTP-Generator` `Express Rate Limit` `XSS-Clean` `Mongo Sanitize`

---

## 📈 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=asmit557&show_icons=true&theme=radical" width="49%" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=asmit557&theme=radical" width="49%" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=asmit557&layout=compact&theme=tokyonight" width="40%" />
</p>

### 🐍 Contribution Snake

![snake gif](https://github.com/asmit557/asmit557/blob/output/github-contribution-grid-snake.svg)

---

## 📌 Coding Profiles

* 💻 [LeetCode – @asmit557](https://leetcode.com/Asmit009/) — `550+ problems solved` | `1600+ rating` | `6 badges`
* 🔍 [GitHub – @asmit557](https://github.com/asmit557)

---

## 🔗 Connect with Me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-asmit557-blue?style=for-the-badge\&logo=linkedin)](https://www.linkedin.com/in/asmitverma01/)

---

## 🏆 Highlights & Achievements

* ✨ Qualified for **e-Yantra 2023 Stage 1** by IIT Bombay
* 🎭 PR Lead for **Culrav** (Cultural Fest) and **Avishkar** (Tech Fest)
* 🌱 Active Member of **Green Club** for sustainable growth initiatives
* 🚀 Lead contributor to projects like eCommerce platforms, Ed-Tech apps, and real-time chat apps

---

## 🌟 Fun Facts

* 🎮 I love playing competitive games.
* 🌍 Big fan of dark UI and minimal design.
* 🧠 I enjoy learning about design systems and scalable architectures.

---

> *“Code like poetry should be short and concise.” – Santosh Kalwar*

---

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=150&section=footer"/>
</p>
